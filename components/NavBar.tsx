    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 ${theme.navBg} transition-colors duration-500`}>
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="group flex items-center gap-3">
          ...
          <span className={`hidden md:block text-xs font-medium border-l pl-3 leading-tight transition-colors duration-500 ${
            theme.heroIsLight
        ...
        <div className="flex items-center gap-6">
          ...
          <Link
            href="/maiju"
            className={`text-sm font-semibold px-3 py-1.5 rounded-full transition-colors hidden sm:block ${
              isMaiju ? theme.navMaijuLinkActive : theme.navMaijuLink
            }`}
          >
            ✦ {tr.nav.strategy}
          </Link>
